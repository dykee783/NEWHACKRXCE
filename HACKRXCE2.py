    ...
    def chalo():
        ...
        def animate():
            for c in itertools.cycle(['|', '/', '-', '\\']) :
                if done:
                   break
                sys.stdout.write('\rhacking in the mantrimall.in server for next colour--------- ' + c)
                sys.stdout.flush()
                time.sleep(0.1)
        ...
        if i in thisway:
           ...
           if((m%2==0 and n%2==0) or (m%2==1 and n%2==1)):
               if current in numbers:
                print("Parity",newperiod + 1,":RED ")
               else:
                print("Parity",newperiod + 1,"  : �, GREEN")
           else:
               if current in numbers:
                print("Parity",newperiod + 1,"  : �, GREEN")
               else:
...
if(expirydate>today):
    ...
    if(now>Third and now<Thirdend):
        ...
        banner='mantrimall.in'
        system(banner)
        #print(f"{yellow}"Hi,Thanks for buyin the hack")
        print("Thanks")
        print("play Time")